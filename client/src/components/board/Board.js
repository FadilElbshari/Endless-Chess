...
const BLACK_QUEEN = document.createElement('div');
WHITE_QUEEN.className = 'square';
WHITE_QUEEN.id = 'wq';
WHITE_QUEEN.draggable = false;
WHITE_QUEEN.style.backgroundImage = pieceImages.w.queen;
BLACK_QUEEN.className = 'square';
BLACK_QUEEN.id = 'bq';
BLACK_QUEEN.draggable = false;
BLACK_QUEEN.style.backgroundImage = pieceImages.b.queen;
// const WHITE = 1;
// const BLACK = 0;
...
// const nums = "12345678";
// @ts-ignore
const Board = ({ allow, type, size, fen, flip = false, color, gameId, getTurn, getLegalMoves, makeMove, undoMove, getFen, checkGameOver }) => {
    const boardRef = useRef(null);
    const squareRefs = useRef({});
    const draggedPieceRef = useRef(null);
    const selectedPieceRef = useRef(null);
    const legalMoveSquaresRef = useRef([]);
    const RookSquaresRefs = useRef([]);
    const RookToSquaresRefs = useRef([]);
    ...
    useEffect(() => {
        socket.on("move_update", (data) => {
            ...
            animateMove(UPDATE, fromSquare, toSquare, enPassantFlagCheck, shortCastleFlag, longCastleFlag, potentialEnPassantCaptureSquare, turn, notation.length === 5);
        });
    }, []);
    // Mini Functions (Getters)
    const removeAllEnteries = () => {
        document.querySelectorAll(".entered").forEach((el) => el.classList.remove("entered"));
    };
    const getSquareId = (move) => {
        ...
        moves.forEach(move => {
            if (fromSquare === move[0]) {
                ...
                if (!square)
                    return;
                squares.push(square);
                const sqrChildNode = square.childNodes[0];
    ...
    const getEnPassantData = (toSquare) => {
        const turn = getTurn();
    ...
    const getAnimatedMoveData = (fromSquare, toSquare) => {
    ...
    // @ts-ignore
    const animateMove = (moveType, fromSquare, toSquare, enPassantFlagCheck, shortCastleFlag, longCastleFlag, enPassantSquare, turn, promotion, isUndo = 0, piece = null, enPassantPiece = null) => {
        const [distanceX, distanceY] = getAnimatedMoveData(fromSquare, toSquare);
        if (moveType === UPDATE) {
            const fromPiece = fromSquare.childNodes[1];
            ...
            setTimeout(() => {
                if (fromSquare.childNodes.length > 1)
                    fromSquare.removeChild(fromSquare.childNodes[1]);
                if (toSquare.childNodes.length > 1)
                    toSquare.removeChild(toSquare.childNodes[1]);
                if (enPassantFlagCheck && enPassantSquare)
                    enPassantSquare.removeChild(enPassantSquare.childNodes[1]);
                if (promotion) {
                    toSquare.appendChild(flip ? WHITE_QUEEN : BLACK_QUEEN);
                }
                else {
            ...
            const parentDSquare = draggedPiece?.parentNode;
            const selectedPiece = selectedPieceRef.current;
            const parentSSquare = selectedPiece?.parentNode;
        ...
        if (shortCastleFlag) {
            ...
            if (!rookSquare || !rookToSquare)
                return;
            const rookPiece = rookSquare.childNodes[1];
    ...
    const handlePointerDown = useCallback((event) => {
        ...
        if (event.pointerType === 'touch' && nativeEvent.touches && nativeEvent.touches.length > 1)
            return;
        ...
        if (selectedPieceRef.current && selectedPieceRef.current === piece) {
            ...
            if (selectedPieceRef.current && selectedPieceRef.current !== draggedPieceRef.current) {
                const pNode = selectedPieceRef.current.parentNode;
                pNode.classList.remove("selected-square");
            }
            selectedPieceRef.current = null;
            removeLegalMoves();
            const parentDSquare = draggedPieceRef.current.parentNode;
    ...
    const handlePointerMove = useCallback((event) => {
        if (!mouseDownRef || !draggedPieceRef.current || !boardRef.current)
            return;
        event.preventDefault();
    ...
    const handlePointerUp = useCallback((event) => {
        ...
        const fromSquare = draggedPieceRef.current.parentNode;
        const result = calculateValuesForPieceMovement(event, boardRef, flip);
        if (!result) {
        ...
        if (!toSquare)
            return;
        let notation = getMoveNotation(fromSquare, toSquare);
        const potentialEnPassantCaptureSquare = getEnPassantData(toSquare);
        draggedPieceRef.current.classList.remove("dragging");
        draggedPieceRef.current.style.cursor = "grab";
        removeLegalMoves();
        ...
        const turn = Number(getTurn() === "w");
        if (toSquare.id[1] === "8" || toSquare.id[1] === "1") {
            if (fromSquare.childNodes[1].id[1] === "p") {
                notation = notation.concat("Q");
            }
        }
        // @ts-ignore
        ...
        if (status) {
            // moveCount++;
            animateMove(DRAG, fromSquare, toSquare, enPassantFlagCheck, shortCastleFlag, longCastleFlag, potentialEnPassantCaptureSquare, turn, notation.length === 5);
            const { isOver, result, quote } = checkGameOver();
            if (isOver)
                console.log("Game Over:", quote, "Result:", result);
        }
    ...
    const handleClickInsideBoard = useCallback((targetSquareId) => {
        ...
        if (!targetSquare)
            return;
        const pieceOnTarget = targetSquare.childNodes.length > 1 ? targetSquare.childNodes[1] : null;
        if (selectedPieceRef.current && getTurn() === color) {
            const fromSquare = selectedPieceRef.current.parentNode;
            let notation = getMoveNotation(fromSquare, targetSquare);
            const potentialEnPassantCaptureSquare = getEnPassantData(targetSquare);
            removeLegalMoves();
            const turn = Number(getTurn() === "w");
            if (targetSquare.id[1] === "8" || targetSquare.id[1] === "1") {
                if (fromSquare.childNodes[1].id[1] === "p") {
                    notation = notation.concat("Q");
                }
            }
            ...
            if (status) {
                //moveCount++;
                animateMove(SELECT, fromSquare, targetSquare, enPassantFlagCheck, shortCastleFlag, longCastleFlag, potentialEnPassantCaptureSquare, turn, notation.length === 5);
                ...
                if (pieceOnTarget && pieceOnTarget.id && pieceOnTarget.id[0] === getTurn() && color === getTurn()) {
                    console.log(color);
                    const parentSSquare = selectedPieceRef.current.parentNode;
                    fromSquare.classList.remove("selected-square");
    ...
    const handleClickOutsideBoard = useCallback((event) => {
        if (!boardRef.current)
            return;
        const boardRect = boardRef.current.getBoundingClientRect();
    ...
    return (_jsx("div", { ref: boardRef, onPointerDown: handlePointerDown, className: "board", style: { width: `${size}vmin`, pointerEvents: `${allow ? "all" : "none"}` }, draggable: false, children: finalBoard.map((row, i) => {
            ...
            return (finalRow.map((square, j) => {
                ...
                return (_jsxs("div", { id: squareId, ref: (el) => { (squareRefs.current[squareId] = el); }, className: `base-square ${isDark ? "dark" : "light"}`, children: [_jsx("div", { className: "move-ind hide" }), pieceData && (_jsx("div", { className: "square", id: pieceData.id, style: { backgroundImage: pieceData.img }, draggable: false }))] }, squareId));
            }));
        }) }));
};
const parseFen = (fen) => {
...
const determinePieceType = (square) => {
    switch (square) {
        case "p":
...
const calculateValuesForPieceMovement = (event, boardRef, boardFlipped) => {
    if (!boardRef?.current)
        return null;
    ...
    if (relativeX < 0 || relativeY < 0 || relativeX > boardW || relativeY > boardW)
        return null;
